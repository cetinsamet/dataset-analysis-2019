...
class Network(nn.Module):
    ...
    def __init__(self, d_in, d_out):
        super().__init__()
        self.layer1     = nn.Linear(d_in, 256)
        self.layer2     = nn.Linear(256, 128)
        self.layer3     = nn.Linear(128, K)
        self.layer4     = nn.Linear(K, d_out)
        self.relu       = nn.ReLU()
...
def main():
    ...
    if K == 2:
        plt.scatter(x_transformed[:, 0], x_transformed[:, 1], s=[2 for _ in range(N_SAMPLE)], c=cols)
        plt.show(...)
    elif K == 3:
        fig = plt.figure()
        ax = Axes3D(fig)
        ax.scatter(x_transformed[:, 0], x_transformed[:, 1], x_transformed[:, 2], s=[2 for _ in range(N_SAMPLE)],
                   c=cols, marker='.')
        plt.show()
    else:
